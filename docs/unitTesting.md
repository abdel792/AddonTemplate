...
Ensuring your add-on and tooling behavior remains consistent during development is done through automated unit testing.
Following template updates is also validated using these tests.

## Running Tests Locally

For unit tests to execute successfully, target modules (such as `syncAddonTool`) must be located at the root of the repository as sibling files to the `tests/` directory (at the same hierarchical level).
